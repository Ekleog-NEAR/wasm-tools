...
impl TypeId {
    pub(crate) fn new(type_size: usize, index: usize, type_index: Option<usize>, is_core: bool) -> TypeId {
        let mut type_hash = 0;
        if is_core {
            type_hash |= 1 << 63;
        }
        if let Some(type_index) = type_index {
            type_hash |= 1 << 62;
            let type_index: u64 = type_index.try_into().expect("failed converting usize to u64");
